...
export default class Rebates extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            openRebates: false,
    ...
    onClick(){
        ...
        this.setState({
            openRebates: !this.state.openRebates,
            rotate: value
        })
    }
    ...
    render(){
        let explanation;
        if(this.state.openRebates===true){
        ...
        return(
            <div id="Rebates" className="extra-paragraph">
            <div>Rebates</div>
            <div onClick={()=>this.onClick()}><i className="fa fa-angle-down" aria-hidden="true" style={{transform: "rotate("+this.state.rotate+"deg)"}}></i></div>
                <div className="float-right minus">$495.00</div>
                    <CSSTransitionGroup
                transitionName="transition"
                transitionEnterTimeout={400}
                transitionLeaveTimeout={300}>{explanation}</CSSTransitionGroup>
    </div>
